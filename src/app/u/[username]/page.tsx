        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            <FormField
              ...
              render={({ field })=>(
                <FormItem>
                <FormLabel>Send Annonymous Message to @{params.username}</FormLabel>
                <FormControl>
                  <Textarea placeholder="Write your annonymous message here" {...field}/>
                  </FormControl>
                  <FormMessage/>
              </FormItem>
              )}
          ...
          <div className='w-full p-2 space-y-8 bg-white rounded-lg shadow-md'>
            <h1 className="text-xl font-extrabold tracking-tight lg:text-5xl mb-6">Messages</h1>
            {
            suggestedMessages.length > 0 ? (
            suggestedMessages.map((message, index)=>(
              <Button key={index} variant="secondary" onClick={()=>(form.setValue('content', message, { shouldValidate: true }))}>{message}</Button>
            ))
          ):(
